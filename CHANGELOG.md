# 更新日志

本文件记录 Belfry 面向用户的版本更新，按版本倒序排列。版本日期以 Git 发布节点为准。

当前整理范围：`v0.6.0` 至 `v0.21.8`。

## [0.21.8] - 2026-09-24

### 修复

- 修复 Windows 上 Agent Hook（belfry-hook）首次冷启动因程序加载与杀毒实时扫描耗时超过原有 1 秒超时、被提前结束，导致钩子「已安装却连不上」、状态无法回传的问题；Windows 下超时放宽至 8 秒，macOS/Linux 保持不变。

## [0.21.7] - 2026-09-24

### 修复

- 修复插件内嵌浏览器在聚焦后粘贴、键盘编辑命令可能被丢弃、无法生效的问题；内嵌浏览器现始终按前台标签页处理，聚焦后的输入可靠送达目标元素。

## [0.21.6] - 2026-09-24

### 修复

- 修复 Windows 打开用户手册、GitHub、更新日志及终端网页链接时，因命令行重复转义而提示“找不到文件”的问题；改为使用系统原生接口直接打开默认浏览器。

## [0.21.5] - 2026-09-24

### 修复

- 修复左侧会话列表过长时撑出窗口、无法使用滚轮滚动的问题；列表在侧栏内滚动，底部操作按钮保持可见。

## [0.21.4] - 2026-09-18

### 新增

- **用户手册文档站**：中英双语手册上线，「关于」页、README 与仓库主页均提供入口。

### 改进

- 「关于」页发布卡片与 Provider 列表按条目数量自适应列宽，不同窗口宽度下对齐更整齐。

### 修复

- 「关于」页的用户手册 / GitHub 仓库 / 更新日志按钮点击无反应：Tauri webview 内 `window.open` 会被静默丢弃，外链改由自写后端命令调用系统原生方式打开，并强制把浏览器激活到前台；打开失败时前端能拿到错误，不再是「点了没反应」的哑故障。

## [0.21.3] - 2026-09-17

### 新增

- **接入 Pi CLI**：工作区支持第三种 CLI Agent，与 Codex、Claude 同等参与分屏、协作与用量统计。
- **「关于」设置页**：查看应用版本与更新状态，并检测三个 CLI 的本地与最新版本，支持一键升级。
- 版本检测从可执行文件反查真实包名：装的是第三方镜像包（如 `@cometix/claude-code`）且滞后于官方包时，在卡片上提示官方版本，而升级动作仍只针对已装的包，不偷偷替换。

### 改进

- Provider 卡片视图重做，加入翻牌启用与销毁碎裂动画。
- 分屏标题栏与侧栏会话行支持 Pi 品牌图标。

### 修复

- 暗色主题下会话图标对比度不足（π 图标糊成黑影）：新增 `--text-icon` 灰阶，各暗色底从 3.1~3.7:1 提升到 4.6~5.5:1，同时修好关于页按钮未右对齐的问题。

## [0.21.2] - 2026-09-16

### 修复

- 安装器增加清理进程的启动兜底并修正错误提示。

## [0.21.1] - 2026-09-16

### 修复

- 检测应用移动或构建清理后失效的 Hook 命令，并提示重新预览和启用，避免将无效 Hook 显示为已启用。
- 未绑定原生会话时，会话统计展示具体的 Hook 状态及恢复建议。
- Git Worktree 列表兼容旧版 Git。

## [0.21.0] - 2026-09-15

### 新增

- **后台终端与命名工作区**：后台进程独立持有终端，重新打开界面时连接原会话；命名工作区保留会话分组、分屏布局和活动焦点。
- **更完整的 Agent 工作流**：加入 Hook 状态、历史全文检索与会话详情、项目级 Provider，以及 Git Worktree 的创建、提交、合并和安全清理入口。
- **项目与连接管理**：支持项目收藏、分组和启动配置、SSH 主机档案、本地工作区与外观备份、自定义快捷键，以及可选的命令收藏插件。
- **用量洞察**：补充趋势、模型与项目明细、费用估算及可配置价格。

### 改进

- 用量查询使用增量缓存、重复请求合并和后台取消，大日志超出索引预算时回退到流式统计；修复取消旧请求时新请求误用已取消任务的竞态。
- 终端按前端实际消费进度投递输出，限制待确认数据，保持后台进程运行；回放超出缓存时显示明确缺口，改进重连、退出排空与输入处理。
- 设置、历史和用量等可选面板按需加载，加载失败可重试；无法局部恢复时保留终端并提供操作提示。
- 统一设置页与控件布局，修复窗口拖拽冲突、重启后的终端焦点、备份主题初始化和 Worktree 无效分支提示。
- macOS / Windows 自动回归成为发布前置检查，三平台安装包、更新包及签名齐备后才公开新版本。

### 验收说明

- Windows 原生交互及部分 Hook / Worktree 桌面场景的剩余验收见 `docs/cli-manager-implementation.md`；自动测试与构建结果不代替这些实机验收。

## [0.20.2] - 2026-09-10

### 修复

- **会话中发现新安装的插件工具**：新增常驻的工具发现与调用入口，让缓存旧工具清单的 Agent 也能直接调用后来安装并启用的插件；停用、卸载或调整作用范围后按最新状态校验。
- **PI 待办面板与 Agent 数据同步**：兼容原版 PI 面板的消息封装，正确转发 `todo.sync` 等插件通道，修复面板新增待办后 Agent 查不到、重新加载后丢失的问题。
- **插件目录通知恢复**：MCP 通知连接断开后自动重连，并在建连与重连时补发工具、提示词和资源目录变更通知。
- 清理两处未使用代码导致的 Rust 编译警告，保留插件安装测试入口。

## [0.20.1] - 2026-09-10

### 修复

- **PI 公开市场插件安装兼容性**：支持市场目录中 `>=0.8.0`、`^0.14.0` 等宿主版本范围声明，裸版本号继续按最低版本处理，修复兼容插件被错误拒绝安装的问题；不兼容或无效的版本要求仍会拦截。
- **安装状态只显示在目标插件上**：预览或安装一个插件时，其他卡片不再一起显示“正在安装”。
- **防止旧快捷键重载工作区**：移除 Recipe 入口后，继续拦截原先的刷新组合键，避免意外重载 WebView 导致终端会话丢失。

### 改进

- 为每个插件市场来源补充用途说明，切换来源时说明与选择器保持在同一行，减少布局跳动。
- 插件工作面板的视图选择器统一为应用内下拉菜单，沿用一致的样式和键盘交互。
- 收紧工作台工具栏间距，并调整窄窗口下的按钮布局。

### 功能调整

- **移除独立 Prompt Composer 与 Recipe 面板**，同时移除对应工具栏按钮、Quick Open 动作和快捷指令入口。后台 Prompt Queue 继续承担会话协作任务的排队与投递，保留忙碌等待、空闲派发和终端重挂恢复能力。

## [0.20.0] - 2026-09-10

### 新增

- **插件中心与 PI 插件系统**：内置自有插件市场，插件可发现、安装、更新。初始上架工作便笺、文本统计、代码审查指南三个插件。插件运行需要本机装有 Node.js 20 或更新版本，没装时已安装的静态插件仍可用。

### 改进

- **Windows 终端字体自带真实字重**：0.19.0 在亮色下把终端文字整体提一档字重来补偿发虚，但那依赖系统装了带 Medium 的 Cascadia Mono。回退到只有 Regular / Bold 两档的 Consolas 时，请求 500 会被字体匹配落回 400，提档等于没发生。这一版随包分发 JetBrains Mono 的真实 Medium 与 SemiBold，Windows 默认等宽字体改为优先使用内置字体，中文回退到同样已随包的 HarmonyOS Sans SC，不再看系统装了什么。macOS 的系统字体栈保持原样，你自己选择或导入的字体仍然优先。
- **换字体、换主题后不再残留上一套字形**：终端首次挂载、切主题、换字体都会等字体真正加载完成，再清理 WebGL 的字形图集并重新测量字号。此前这几个时机可能把字体尚未就绪时生成的回退字形一直缓存下去。
- 插件相关的下拉菜单统一为一套样式与键盘交互。

### 修复

- **Windows 版无法构建**：插件系统引入的两处代码调用了只在 macOS 下可见的函数，Windows 目标编译报错。0.19.0 之后的 Windows 构建都受影响。

## [0.19.0] - 2026-09-06

### 修复

- **亮色模式终端文字发虚**：实测下来是四层原因叠加，只在亮色 + 背景图开启时同时发作。逐像素量到的对比度只有 4.11:1（低于 WCAG AA 的 4.5:1），而主题前景色本应有 14.29:1——说明发虚的那些文字走的是被刻意压浅的 ANSI 色号，不是前景色。这一版从三处一起治：
  - 亮色下启用 xterm 的对比度兜底（`minimumContrastRatio`），把对比度不够的 ANSI 色号往深处拉一档。此前这个开关是关着的（默认值即关闭）。暗色保持关闭，配色不受影响。
  - 亮色调色板里色号 37（常被程序用来输出正文）压深到 8.21:1。
  - 亮色下正文与粗体字重各提一档，补偿灰度抗锯齿在浅底上把笔画显细的那一档。顺带修掉一个既有问题：粗体原先设的 500 在只有 Regular/Bold 两档的等宽字体上会落回 400，也就是粗体和正文渲染得一模一样。
- **背景图衬底浓度改为亮暗各记一份**：亮色默认提到 80%，让文字底下接近纯色，消掉背景图案透上来的高频噪声；暗色保持 55% 不变。旧配置里的单一数值会继承给暗色，亮色取新默认值。
- 协作面板里的任务短 ID 在亮色下几乎看不见（最弱的一档文字色上又叠了 50% 不透明度），而它是要逐字认出来复制的。

## [0.18.0] - 2026-09-03

### 改进

- **协作面板改为时间轴事件流**：派出去的活不再是一列等重的卡片，而是轨道上的一个个事件。等你确认、进行中、已完成、对方做不了、目标会话没了各有自己的节点形态，进行中的那条会呼吸。
- **看得出派出去多久了**：任务新增时间显示（刚刚 / N 分钟前 / N 小时前 / 时刻 / 昨天 / 日期）。此前面板完全没有时间信息，看到「已送出 · 完成情况未知」也不知道是 5 秒前还是 20 分钟前。
- **对方交差时说的话提到显眼位置**：完成或失败时对方留下的那句解释，从底部一行小字改为带引导线的引用块——那是对方唯一能给出的说明。
- **空态改为虚线轨道预演**：还没有派活记录时，用一条虚线轨道预演「派完活这里会长成什么样」，两条示例命令支持点击复制。

### 修复

- 任务较多时页脚的「全部停下」会被挤出可视区：面板布局写死了四条栅格轨道，但错误提示条是按需渲染的，没有错误时那条自适应轨道落到了页脚上。
- 背景图开启时协作面板没有毛玻璃层，是一块不透明面板硬贴在背景图上，和历史、用量两个面板不是一套观感。
- 进入设置页时协作面板不会被隐藏（样式里的类名拼写与实际不符）。

## [0.17.0] - 2026-09-01

### 新增

- **双 Agent 环境诊断**：协作环境检查扩展到 Codex 与 Claude Code，分别展示 CLI 版本、登录状态，以及客户端支持的专项检查。
- **多客户端 Skill 同步**：启动时和手动操作都会同步 Codex、Claude Code 两端的内置 `belfry` skill，并分别返回安装、更新或已是最新的结果。

### 改进

- Skill 同步支持部分成功：单个客户端目录不可用时保留另一端的成功结果，并明确显示失败客户端与原因。
- 未安装的可选 Agent 只显示警告，不再把仅使用 Codex 或仅使用 Claude Code 的环境误判为阻塞。

## [0.16.0] - 2026-09-01

### 新增

- **协作环境诊断**：在设置中集中检查 Belfry skill、Codex CLI 版本与登录状态、`multi_agent` 功能、`codex doctor` 结果，以及内置控制 CLI 与协作服务的端到端连通性。
- **Skill 生命周期管理**：应用启动时自动同步内置 `belfry` skill，也可在协作环境页面手动安装或更新；诊断会区分未安装、需要更新和已同步状态。
- **可视化检查结果**：按正常、警告和异常汇总环境状态，展示每项检查的具体结论，并支持一键重新检查。

### 改进

- 协作通道自检使用临时身份并在结束后立即撤销，避免诊断凭证残留在当前应用实例中。

## [0.15.0] - 2026-09-01

### 新增

- **会话协作**：Agent 会话可设置稳定且唯一的协作名字，通过 `belfry peers`、`send`、`inbox`、`wait`、`done` 与 `fail` 在现有会话之间派活、等待和交差。
- **协作控制面板**：集中查看待确认、进行中和已结束任务，支持逐条批准 / 拒绝、一键停止，并在工作台入口显示待确认数量。
- **安全闸门**：只允许同项目 Agent 会话互相派活，限制转包深度和单轮消息数，拦截自派、成环、失效目标与跨项目请求；会话身份由后端签发，凭证不经过前端。
- **可靠投递**：复用 Prompt Queue，在目标忙碌、等待确认或终端重挂时保持任务顺序；目标退出后任务自动作废，只有接收方提交的 `done` / `fail` 才作为可信完成信号。
- **内置控制 CLI**：将 `belfry` 作为 Tauri sidecar 随 macOS / Windows 安装包分发，并仅注入 Belfry 托管的 Agent 终端环境。

### 修复

- 为从 GUI 启动的 Agent 终端补齐 UTF-8 locale，避免中文输出按单字节编码显示异常。

## [0.14.0] - 2026-08-20

### 新增

- **Agent adapter foundation**：将 Codex / Claude 的检测、启动、resume、历史扫描与清理统一到适配器契约，集中描述能力和 Agent 身份。
- **会话身份与恢复安全**：历史会话携带显式 Agent + session reference，恢复参数和工作区存档在边界处校验，避免跨 Agent 冲突和路径型 session id。
- **生命周期契约**：提供版本化的 Agent 状态快照、生命周期事件和恢复计划 DTO，为后续结构化 hooks 与 Recipe 回放预留稳定接口。
- **快捷指令面板优化**：改为更宽的双列布局，固定快捷键 / 命令值列，补充分组计数、上下文提示与窄屏响应式排版，避免内容被压缩。

### 修复

- 修复历史操作在 Agent 切换和首屏 Agent 检测期间的异步竞态，防止使用过期 Agent 上下文恢复会话。

## [0.13.0] - 2026-08-19

### 新增

- **File Preview Pane**：从当前项目浏览目录，打开只读文本预览；文件树按目录优先排序并过滤常见生成目录。
- **安全文件服务**：预览路径由 Rust 校验在项目根目录内，限制单文件读取大小，二进制文件显示不可预览提示。
- **轻量代码高亮**：为常见 Rust、TypeScript、JavaScript、JSON、Markdown、CSS 等文本提供关键词、字符串、数字和注释层次。
- **变更提醒**：文件在磁盘上被修改后，预览区提示重新加载，不会静默覆盖当前阅读位置。
- **工作区入口**：新增文件预览按钮、Quick Open 动作和终端路径链接，预览窗格可独立调整宽度并定位到输出中的行号。

## [0.12.0] - 2026-08-19

### 新增

- **Prompt Composer**：通过 `⌘J`（Windows / Linux 为 `Ctrl+Shift+J`）打开独立编辑器，选择 Codex 或 Claude 会话并提交多行 Prompt。
- **Prompt Queue**：Agent 正在输出或等待确认时，Prompt 按会话分别进入队列；回到 `running + idle` 后按提交顺序逐条发送。
- **队列恢复**：终端目标尚未注册、发送失败或 xterm 重挂时保留未确认 Prompt；关闭会话时自动清理该会话队列。
- **工作区入口**：Quick Open 和快捷指令面板都新增 Prompt Composer 动作。

### 设计边界

- Composer 只驱动 Codex / Claude Agent 会话，不向 Shell 或 SSH 会话注入命令。
- 队列暂存于当前应用内存，关闭 Belfry 后不会恢复；发送仍走 xterm 的 paste + 回车输入链路。

## [0.11.0] - 2026-08-19

### 新增

- **Quick Open**：通过 `⌘K`（Windows / Linux 为 `Ctrl+Shift+K`）打开快速操作面板。
- **统一搜索**：可按会话名、项目路径、Agent 类型和动作关键词模糊搜索，支持中文、大小写不敏感和多词过滤。
- **键盘导航**：上下键循环选择，Home / End 跳转，Enter 执行，Escape 关闭；当前结果会自动滚入可视区域。
- **快速动作**：直接切换会话、打开最近项目、新建 Shell、打开设置 / 历史 / 用量 / 快捷指令，或收起侧栏。

## [0.10.0] - 2026-08-19

### 新增

- **跨平台 Shell Profile**：macOS 支持系统默认、zsh、bash、fish；Windows 支持 PowerShell 7、Windows PowerShell、CMD、WSL、Git Bash，并按固定 allowlist 检测和启动。
- **终端搜索**：`Ctrl/Cmd+F` 打开搜索浮层，支持大小写不敏感、前后匹配、匹配计数、CJK 宽字符和跨 wrapped line 搜索。
- **HTTP(S) 链接**：自动识别终端中的网址，清理句末标点后可点击并在新窗口打开。
- **Unicode 增强**：注册 xterm Unicode provider，改善 CJK、组合字符、emoji、ZWJ emoji 和旗帜显示。
- **会话恢复兼容**：工作区存档保存 Shell Profile，并兼容旧版本存档；SSH 目标继续持久化但不写入密码。

## [0.9.0] - 2026-08-17

### 新增

- **字体与字号管理**：在外观设置中统一调整应用字体和 `10–20px` 字号，并即时预览。
- **字体导入**：支持导入多个 TTF、OTF、WOFF、WOFF2 字体，分别切换、删除，或恢复系统字体。
- **快捷指令系统**：新增全局快捷键注册、会话快速切换、新建 Shell、打开设置和历史面板等操作。
- **快捷指令面板**：通过 `⌘/`（Windows / Linux 为对应的 `Ctrl+Shift` 组合）查看当前平台可用快捷键。

## [0.8.0] - 2026-08-17

### 新增

- **SSH 会话**：从新会话菜单直接启动系统 OpenSSH，继承 `~/.ssh/config`、密钥和 SSH agent。
- **SSH 认证辅助**：支持密码、主机指纹和 2FA 的终端交互；勾选记住密码后，凭据保存到 macOS Keychain 或 Windows 凭据管理器。
- **凭据管理**：可在 SSH 表单中清除已保存的密码。

### 修复

- 修复 Windows 上 OpenSSH 可执行文件的路径解析，避免 SSH 入口检测失败。

## [0.7.0] - 2026-08-16

### 新增

- **历史会话管理**：读取本地 Codex / Claude Code 会话日志，支持列表查看、恢复会话、删除单条或批量清空。
- **Provider 配置编辑器**：在设置中新增 Provider 编辑、配置预览、校验和刷新能力，继续只改写 CLI 配置中的路由字段。
- **背景显示控制**：新增文字衬底调节，并优化背景图片的透明度和模糊效果。
- **透明主题同步**：Codex 透明模式下的终端样式、背景和滚动条跟随应用主题同步。

### 修复

- 修复 Shell 内 Codex 透明主题同步不完整的问题。

## [0.6.0] - 2026-08-15

### 新增

- **Provider 切换**：在官方端点与第三方中转之间切换 Codex / Claude Code，并精准改写 CLI 配置中的路由字段。
- **配置保护**：切换 Codex 第三方 Provider 前备份 ChatGPT 登录态，切回官方时恢复；同时提示会覆盖配置文件的环境变量。
- **活动通知**：Agent 完成任务或等待确认时发送系统通知，并在应用内汇总未读数。

### 修复

- 修复 Windows 构建中错误复用 macOS 专属 Agent 命令路径的问题。

## 版本链接

[0.20.1]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.20.1
[0.12.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.12.0
[0.13.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.13.0
[0.14.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.14.0
[0.15.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.15.0
[0.16.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.16.0
[0.17.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.17.0
[0.11.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.11.0
[0.10.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.10.0

[0.9.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.9.0
[0.8.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.8.0
[0.7.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.7.0
[0.6.0]: https://github.com/ChengSoon/belfry-desktop/releases/tag/v0.6.0
