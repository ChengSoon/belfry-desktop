...
fn a_task_records_the_whole_chain() {
    let parent = task_at("t1", "t2", 1, &["t1", "t2"]);
    let task = build_task("id-2".into(), &request("t2", "t3", Some(&parent)), 100);
...
fn a_first_hand_task_starts_the_chain_with_its_sender() {
    let task = build_task("id-1".into(), &request("t1", "t2", None), 100);
...
fn the_injection_stays_three_lines() {
    let task = build_task("01ABCDEFGH".into(), &request("t1", "t2", None), 0);
...
fn closing_a_session_abandons_its_open_tasks() {
    let board = TaskBoard::default();
    board.insert(build_task("id-1".into(), &request("t1", "t2", None), 0));
    board.insert(build_task("id-2".into(), &request("t1", "t3", None), 0));
    board
...
fn the_run_counter_follows_the_chain_root() {
    let board = TaskBoard::default();
    let first = build_task("id-1".into(), &request("t1", "t2", None), 0);
    let second = build_task("id-2".into(), &request("t2", "t3", Some(&first)), 0);
    board.insert(first);
    board.insert(second);
    board.insert(build_task("id-3".into(), &request("t9", "t8", None), 0));
