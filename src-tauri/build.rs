...
fn main() {
    println!("cargo:rustc-check-cfg=cfg(belfry_cross_check)");
    if std::env::var_os("BELFRY_CROSS_CHECK").is_some() {
        println!("cargo:rustc-cfg=belfry_cross_check");
        return;
    }
    stage_conpty();
    tauri_build::build()
...
/// 把架构对得上的那份 conpty 摆到 `Belfry.exe` 旁边。
///
...
/// target 目录给 `tauri dev`（跑的是 `target/<profile>/belfry-desktop.exe`）。
///
