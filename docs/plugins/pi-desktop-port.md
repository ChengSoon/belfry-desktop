...
## 2026-09-10 会话工具发现与 PI 待办同步

用户实测发现：实时 MCP 服务已有 `pi.todo/todo_manage`，Agent 提供给模型的工具清单仍缺少
该工具；原版待办面板显示新增任务后，Agent 查询和共享设置文件中却没有这条任务。
本轮只修复宿主接入，保留从 PI 市场安装的原版插件。

- 从连接初始化起提供 `PluginTools` 和 `PluginCall`，每次读取当前工作区的实时工具、
  参数格式与作用范围，再调用原工具。缓存原生工具清单的客户端也能使用后来启用的插件。
  初始化说明引导 Agent 优先走工具；停用、卸载和收窄范围会立即阻止旧名称调用。
- stdio 桥在初始化完成后订阅目录事件，连接中断按有界退避重连；建连和重连补发
  tools/prompts/resources 目录通知，覆盖订阅空窗。会话授权失效后停止重连。
  客户端自身工具面板的刷新能力仍由客户端实现，常驻入口提供实际可调用的兼容路径。
- 将 PI 的 `skill.setEnabled` 包装解开，以 `payload.id` 调用本插件的 `onPanelInvoke`，
  完整保留 payload。空 ID 与宿主管理通道被拒绝；直接自定义通道继续工作。
  原版 `pi.todo 0.6.5` 因此能将 `todo.sync` 保存到与 Agent 共用的设置文件。

新增回归先复现缺失入口及 `unsupported panel channel: skill.setEnabled`，修复后验证旧会话
新增/卸载插件、范围变化、MCP 内容保留、通知断线恢复、面板写入和插件重载后的持久化。
原版待办在临时数据目录中验证双向共享；其 0.6.5 面板按上游行为在启动和重新聚焦时拉取数据。

最终验证：Node 全量 145 项通过，起初缺少参考包的 3 项已补齐并单独通过，合计覆盖 148 项；
包括原版 Todo、Browser、Git Lens 0.2.7、Log Viewer 0.2.0，下载包经过市场 SHA-256 校验。
最后的函数拆分后，MCP 与面板定向 20 项、Rust 嵌入运行时 3 项再次通过。
Vitest 80 文件、547 项通过；Rust 全量 343 项库测试和 1 项集成通过，4 项原有忽略。
10 个相关 JS 文件、103 个函数完成 AST 结构检查，无文件/函数长度、参数、嵌套或复杂度违规。
Rust 格式与差异检查通过；顺手按格式工具调整了 `provider/mod.rs` 的模块声明顺序。

最终 macOS 构建成功，产物为 `src-tauri/target/release/bundle/macos/Belfry.app`，使用本地临时
